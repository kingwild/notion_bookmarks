...
async function read(url: string) {
  ...
  if (new URL(url).hostname === 'www.52pojie.cn') return new TextDecoder('gbk').decode(await result.arrayBuffer());
  return result.text();
}
...
export async function loadFeed(source: NewsSourceId): Promise<NewsItem[]> {
  ...
  switch (source) {
    ...
    case 'baidu': {
      ...
      items = root.querySelectorAll('.category-wrap_iQLoo').map(item => ({ title: item.querySelector('.c-single-text-ellipsis')?.text.trim() || '', url: item.querySelector('a')?.getAttribute('href') || '', detail: item.querySelector('.hot-index_1Bl1a')?.text.trim(), excerpt: item.querySelector('.hot-desc_1m_jR.large_nSuFU')?.text.replace(/查看更多.*$/, '').trim() })); break;
    }
    case 'bilibili': {
      const data = JSON.parse(await read('https://api.bilibili.com/x/web-interface/popular?ps=50&pn=1'));
      items = (data.data?.list || []).map((item: { title: string; bvid: string; desc?: string; owner?: { name: string } }) => ({ title: item.title, url: `https://www.bilibili.com/video/${encodeURIComponent(item.bvid)}`, detail: item.owner?.name, excerpt: item.desc })); break;
    }
    case 'toutiao': {
      const data = JSON.parse(await read('https://www.toutiao.com/hot-event/hot-board/?origin=toutiao_pc'));
      items = (data.data || []).map((item: { Title: string; HotValue: number; Url?: string }) => ({ title: item.Title, url: safeUrl(item.Url || '') || `https://www.toutiao.com/search/?keyword=${encodeURIComponent(item.Title)}`, detail: heat(item.HotValue) })); break;
    }
    case 'douyin': {
    ...
    case 'github': {
      ...
      items = (data.items || []).map((item: { full_name: string; html_url: string; description: string; stargazers_count: number }) => ({ title: item.full_name, url: item.html_url, detail: `★ ${item.stargazers_count.toLocaleString()} · ${item.description || ''}`, excerpt: item.description })); break;
    }
    case 'tools': {
      const root = parse(await read('https://www.producthunt.com/feed'));
      items = root.querySelectorAll('entry').map(item => ({ title: item.querySelector('title')?.text.trim() || '', url: item.querySelector('link')?.getAttribute('href') || '', detail: item.querySelector('published')?.text.slice(0, 10), excerpt: parse(item.querySelector('content')?.text || '').text.replace(/<[^>]*>/g, '').trim() })); break;
    }
    case 'industry': {
