...
function NewsCard({ source, refresh }: { source: typeof NEWS_SOURCES[number]; refresh: number }) {
  ...
  useEffect(() => {
    ...
    const manual = retry !== lastRetry.current; lastRetry.current = retry;
    setLoading(true); setNotice(manual ? '正在向来源请求最新榜单…' : '');
    requestJson<NewsFeed>(`/api/workspace/news?source=${source.id}${manual ? '&refresh=1' : ''}`, { signal: controller.signal, cache: 'no-store' })
      .then(data => {
        if (controller.signal.aborted) return;
        if (data.items.length) setFeed(data);
        else setFeed(previous => previous?.items.length ? { ...previous, stale: true, message: data.message || '更新失败，暂时保留上次内容' } : data);
        setNotice(data.stale || data.status !== 'ok' ? data.message || '更新失败，请重试' : data.refreshAfter ? `刚刚已更新，请 ${data.refreshAfter} 秒后再刷新` : manual ? '已重新获取榜单；没有新内容时排名保持不变' : '');
      }).catch(() => {
        if (!controller.signal.aborted) { setNotice('更新失败，请稍后重试；已有内容会保留'); setFeed(previous => previous || { items: [], fetchedAt: '', status: 'unavailable' }); }
      }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [source.id, refresh, retry]);
  return <article className="ws-card ws-news-card"><header><span className="ws-source-mark" style={{ background: source.color }}>{source.mark}</span><div><h3>{source.name}</h3><p>{source.label}</p></div><div className="ws-news-header-actions"><button className="ws-text-button" disabled={loading} aria-label={`刷新${source.name}榜单`} onClick={() => setRetry(v => v + 1)}><RefreshCw className={loading ? 'ws-spin' : ''} size={13} />{loading ? '更新中' : '刷新'}</button><a href={source.url} target="_blank" rel="noreferrer" aria-label={`打开${source.name}`}><ArrowUpRight size={16} /></a></div></header>
    {loading && !feed ? <div className="ws-news-loading" aria-label="加载榜单中">{[1,2,3,4,5].map(n => <div key={n} />)}</div> : feed?.items.length ? <ol tabIndex={0} aria-label={`${source.name}榜单，可上下滚动`}>{feed.items.slice(0, 50).map((item, index) => <li key={item.url}><span className={index < 3 ? 'ws-rank-top' : ''}>{String(index + 1).padStart(2, '0')}</span><button className="ws-news-title" aria-expanded={selected?.url === item.url} aria-controls={`news-${source.id}-${index}`} onClick={() => setSelected(selected?.url === item.url ? null : item)} title={item.detail || item.title}>{item.title}</button>{selected?.url === item.url && <NewsOverview item={item} source={source.id} id={`news-${source.id}-${index}`} onClose={() => setSelected(null)} />}</li>)}</ol> : <div className="ws-feed-empty"><Globe2 size={24} /><p>{feed?.message || '暂时无法读取榜单'}</p><span>可以前往原站查看最新内容</span><div><button disabled={loading} onClick={() => setRetry(v => v + 1)}>重试</button><a href={source.url} target="_blank" rel="noreferrer">打开原站 <ArrowUpRight size={12} /></a></div></div>}
    <footer className="ws-feed-footer"><span>{feed?.items.length ? `${feed.stale ? '上次内容 · ' : ''}共 ${feed.items.length} 条 · ${new Date(feed.fetchedAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}` : '独立数据源'}</span><a href={source.url} target="_blank" rel="noreferrer">原站 <ArrowUpRight size={11} /></a><p role="status">{loading ? '正在更新…' : notice}</p></footer></article>;
}
...
function Notes({ enabled }: { enabled: boolean }) {
  ...
  async function submit() {
    ...
    try { await requestJson('/api/workspace/notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, requestId, nickname }) }); setText(''); setRequestId(crypto.randomUUID()); setStatus('留言已发送，谢谢你的分享！站主会在后台查看。'); }
    catch (error) { setStatus(error instanceof Error ? error.message : '保存失败，草稿已保留'); }
    finally { setBusy(false); }
  }
  return <section className="ws-card ws-notes" id="notes"><div className="ws-section-heading"><div><span className="ws-eyebrow">HELLO, VISITOR</span><h2><StickyNote size={20} /> 给站主留个言</h2></div><span className="ws-note-badge">访客留言</span></div><p className="ws-muted">欢迎来访！推荐一个好网站、提个建议，或简单打声招呼。</p><label className="ws-sr-only" htmlFor="note-content">访客留言内容</label><textarea id="note-content" value={text} disabled={busy} onChange={e => { setText(e.target.value); setRequestId(crypto.randomUUID()); setStatus(''); }} placeholder="你发现了什么好工具？有什么建议，或者想对站主说的话…" maxLength={5000} onKeyDown={e => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') { e.preventDefault(); if (enabled) void submit(); } }} /><div className="ws-note-meta"><span>草稿自动保存在此浏览器</span><span>{text.length} / 5000</span></div><div className="ws-note-actions"><input aria-label="留言昵称（选填）" placeholder="你的昵称（选填）" maxLength={40} value={nickname} onChange={e => setNickname(e.target.value)} /><button className="ws-primary" disabled={!enabled || !text.trim() || busy} onClick={submit}>{busy ? <Loader2 className="ws-spin" size={15} /> : <Send size={15} />}{busy ? '正在发送' : '发送留言'}</button></div><p className="ws-note-status" role="status">{status || (enabled ? '无需登录即可留言，仅站主查看，不会公开展示。' : '留言服务暂时不可用，草稿会保留。')}</p></section>;
}
...
export default function Workspace({ links, categories, title, defaultCity, initialStocks, notesEnabled }: Props) {
  ...
  async function toggleFeatured(link: Link) { if (featuredLock.current) return; featuredLock.current = true; setFeaturedBusy(link.id); setFeaturedError(''); try { const value = !(featured[link.id] ?? link.featured ?? false); await requestJson('/api/workspace/featured', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: link.id, featured: value }) }); setFeatured(previous => ({ ...previous, [link.id]: value })); setFeaturedError(value ? `已将「${link.name}」加入精选并同步` : `已取消「${link.name}」的精选标记`); } catch (e) { setFeaturedError(e instanceof Error ? e.message : '保存失败'); } finally { featuredLock.current = false; setFeaturedBusy(''); } }
  ...
  return <div className={`king-workspace ${dark ? 'ws-dark' : ''}`}><div className="ws-ambient" aria-hidden="true" /><header className="ws-topbar"><a className="ws-brand" href="#top"><span className="ws-brand-symbol">K<span>·</span></span><span>KING<span className="ws-brand-light"> / 工作基地</span></span></a><nav aria-label="页面导航"><a className="ws-nav-active" href="#navigation"><LayoutGrid size={14} /> 导航</a><a href="#discover">发现</a><a href={owner ? '#my-capture' : '#notes'}>{owner ? '随手记' : '访客留言'}</a></nav><div className="ws-top-actions"><span className="ws-top-status"><span className="ws-dot" /> MY PERSONAL SPACE</span><button className="ws-icon-button" aria-label={dark ? '切换浅色外观' : '切换深色外观'} onClick={() => { setDark(!dark); try { localStorage.setItem('king:appearance', dark ? 'light' : 'dark'); } catch {} }}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button><OwnerLogin owner={owner} onChange={() => { setOwner(false); setOwnerRefresh(v => v + 1); }} /></div></header>
    <main className="ws-main" id="top"><section className="ws-welcome"><div><div className="ws-eyebrow">A LITTLE SPACE. A LOT OF POSSIBILITIES.</div><h1>在这里，开启你的每一天<span>。</span></h1><p>常用网站、值得关注的新鲜事，还有下一刻的灵感。</p></div><a href={owner ? '#my-capture' : '#notes'} className="ws-welcome-link"><Plus size={15} /> {owner ? '记录一个想法' : '给站主留言'}</a></section>
      ...
      <section className={`ws-card ws-navigation ${expanded ? 'is-expanded' : ''}`} id="navigation"><div className="ws-section-heading"><div><span className="ws-eyebrow">YOUR DAILY SHORTCUTS</span><h2><Compass size={21} /> 网站导航 <span className="ws-count">{links.length}</span></h2></div><div className="ws-search"><Search size={17} /><input ref={searchRef} value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索网站、工具或灵感…" aria-label="搜索网站" />{query ? <button aria-label="清空搜索" onClick={() => setQuery('')}><X size={14} /></button> : <kbd><Command size={11} /> K</kbd>}</div></div>
        <div className="ws-bookmarks-body"><aside className="ws-categories" aria-label="网站分类">{['全部', ...categoryNames].map((name, index) => <button key={name} aria-pressed={category === name} className={category === name ? 'active' : ''} onClick={() => { setCategory(name); setSub('全部'); }}><span>{index === 0 ? <LayoutGrid size={16} /> : index % 3 === 0 ? <Globe2 size={16} /> : index % 3 === 1 ? <Sparkles size={16} /> : <Bookmark size={16} />}{name === '全部' ? '全部网站' : name}</span><small>{name === '全部' ? links.length : links.filter(link => link.category1 === name).length}</small></button>)}<div className="ws-sidebar-note"><span className="ws-dot" /> 来自你的 Notion 收藏</div></aside>
          <div className="ws-links-area"><div className="ws-subcategories"><button className={onlyFeatured ? 'active' : ''} aria-pressed={onlyFeatured} onClick={() => setOnlyFeatured(v => !v)}>★ 精选</button><button className={sub === '全部' ? 'active' : ''} onClick={() => setSub('全部')} aria-pressed={sub === '全部'}>全部</button>{subs.map(name => <button key={name} className={sub === name ? 'active' : ''} aria-pressed={sub === name} onClick={() => setSub(name)}>{name}</button>)}</div><div className="ws-result-label"><span>{query ? `“${query}” 的搜索结果` : category === '全部' ? '好工具，让工作轻一点。' : category}</span><span>{filtered.length} 个网站</span></div><div className="ws-feature-help">{owner ? <span>点击网站下方的“加入精选”，会同步到 Notion，其他设备也能看到。</span> : <><span>精选由站主管理。登录后可为网站添加或取消精选。</span><OwnerLogin owner={false} label="登录管理精选" onChange={() => { setOwner(false); setOwnerRefresh(v => v + 1); }} /></>}</div>{featuredError && <p className="ws-muted" role="status">{featuredError}</p>}<div className="ws-link-grid">{filtered.map(link => { const url = safeUrl(link.url); if (!url) return null; return <div className={`ws-link-wrap ${owner ? 'ws-link-wrap-owner' : ''}`} key={link.id}><a className="ws-link" href={url} target="_blank" rel="noreferrer"><LinkIcon link={link} /><div><h3>{link.name}</h3><p>{link.desc || link.category2 || new URL(url).hostname}</p><span>{link.category2 || link.category1}</span></div><ArrowUpRight className="ws-link-arrow" size={14} /></a>{owner ? <button className={`ws-feature-action ${(featured[link.id] ?? link.featured) ? 'is-featured' : ''}`} aria-label={`${(featured[link.id] ?? link.featured) ? '取消精选' : '设为精选'}：${link.name}`} aria-pressed={featured[link.id] ?? link.featured ?? false} disabled={!!featuredBusy} onClick={() => toggleFeatured(link)}>{featuredBusy === link.id ? <><Loader2 size={13} className="ws-spin" />正在同步</> : <><Bookmark size={13} fill={(featured[link.id] ?? link.featured) ? 'currentColor' : 'none'} />{(featured[link.id] ?? link.featured) ? '已精选 · 取消' : '加入精选'}</>}</button> : (featured[link.id] ?? link.featured) ? <span className="ws-feature-button is-featured" title="精选网站">★</span> : null}</div>; })}</div>{!filtered.length && <div className="ws-empty"><Search size={28} /><h3>{links.length ? '没有找到匹配的网站' : '这里等待你的第一份收藏'}</h3><p>{links.length ? '试试其他关键词，或换个分类看看。' : '在 Notion 导航链接中添加网站后，它们会出现在这里。'}</p><button className="ws-text-button" onClick={() => { setQuery(''); setCategory('全部'); setSub('全部'); setOnlyFeatured(false); }}>查看全部</button></div>}</div></div>
      ...
      <section className="ws-discover" id="discover"><div className="ws-section-heading"><div><span className="ws-eyebrow">A WINDOW TO THE WORLD</span><h2>正在发生，值得发现<span className="ws-heading-dot">·</span></h2></div><span className="ws-muted ws-feed-hint">每榜独立刷新 · 每 5 分钟自动更新</span></div><div className="ws-news-tabs" aria-label="资讯分类">{[['all','全部发现'],['social','全网热榜'],['industry','工业观察'],['brand','品牌传播'],['tools','开发与工具']].map(([value, name]) => <button key={value} onClick={() => setGroup(value)} className={group === value ? 'active' : ''} aria-pressed={group === value}>{name}</button>)}</div><div className="ws-news-grid">{selectedSources.map(source => <NewsCard key={source.id} source={source} refresh={refresh} />)}</div></section>
