...
function MarketsCard({ initialStocks }: { initialStocks: StockIdentity[] }) {
  ...
  useEffect(() => { try { const saved = JSON.parse(localStorage.getItem('king:stocks') || localStorage.getItem('stockWatchlist:v1') || 'null'); if (Array.isArray(saved) && saved.length && saved.every(s => typeof s.secid === 'string' && /^(?:[01]\.\d{6}|116\.\d{5})$/.test(s.secid) && typeof s.name === 'string')) setStocks(saved.slice(0, 10)); } catch {} }, []);
  useEffect(() => {
  ...
  const save = (value: StockIdentity[]) => { setStocks(value); try { localStorage.setItem('king:stocks', JSON.stringify(value)); } catch {} };
  return <section className="ws-card ws-markets"><div className="ws-card-top"><span className="ws-widget-title"><TrendingUp size={16} /> 自选行情</span><button className="ws-icon-button" aria-label="管理自选股票" aria-expanded={editing} onClick={() => setEditing(!editing)}>{editing ? <X size={16} /> : <Plus size={16} />}</button></div>
    {editing ? <div className="ws-stock-editor"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索股票名称或代码" aria-label="搜索股票" maxLength={40} autoFocus /><div className="ws-stock-chips">{stocks.map(stock => <button key={stock.secid} disabled={stocks.length === 1} onClick={() => save(stocks.filter(item => item.secid !== stock.secid))}>{stock.name}<X size={11} /></button>)}</div><div className="ws-stock-results" aria-live="polite">{searching ? '正在搜索…' : searchError || (query && !results.length ? '没有找到匹配的股票' : '')}{results.map(stock => <button key={stock.secid} disabled={stocks.length >= 10 || stocks.some(s => s.secid === stock.secid)} onClick={() => { save([...stocks, stock]); setQuery(''); }}><span>{stock.name} <small>{stock.code}</small></span><Plus size={14} /></button>)}</div><small className="ws-muted">最多 10 个，自选列表保存在此浏览器</small></div> : <><div className="ws-stock-list" tabIndex={0} role="region" aria-label="自选股票，可上下滚动">{stocks.map(stock => { const quote = quotes.find(q => q.secid === stock.secid); const change = quote?.changePercent; return <a key={stock.secid} className="ws-stock-row" href={`https://quote.eastmoney.com/${stock.secid.startsWith('116.') ? 'hk/' : stock.secid.startsWith('1.') ? 'sh' : 'sz'}${stock.code}.html`} target="_blank" rel="noreferrer"><span>{stock.name}<small>{stock.code}</small></span><strong>{quote?.price?.toFixed(2) ?? '—'}</strong><span className={`ws-change ${change == null ? '' : change >= 0 ? 'up' : 'down'}`}>{change == null ? '—' : `${change >= 0 ? '+' : ''}${change.toFixed(2)}%`}</span></a>; })}</div><div className="ws-market-footer"><span>{loading ? '正在更新…' : error || `腾讯行情 · ${quotes[0]?.updatedAt ? new Date(quotes[0].updatedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Shanghai' }) : '时间未知'} · 可能延迟`}</span><button className="ws-icon-button" aria-label="刷新行情" disabled={loading} onClick={() => setRefresh(v => v + 1)}><RefreshCw size={12} /></button></div></>}
  </section>;
}
...
export default function Workspace({ links, categories, title, defaultCity, initialStocks, notesEnabled }: Props) {
  ...
  const filtered = useMemo(() => links.filter(link => (category === '全部' || link.category1 === category) && (sub === '全部' || link.category2 === sub) && `${link.name} ${link.desc} ${link.tags.join(' ')} ${link.category1} ${link.category2}`.toLowerCase().includes(query.trim().toLowerCase())), [links, category, sub, query]);
  const selectedSources = NEWS_SOURCES.filter(source => group === 'all' || source.group === group);
  return <div className={`king-workspace ${dark ? 'ws-dark' : ''}`}><div className="ws-ambient" aria-hidden="true" /><header className="ws-topbar"><a className="ws-brand" href="#top"><span className="ws-brand-symbol">K<span>·</span></span><span>KING<span className="ws-brand-light"> / 工作基地</span></span></a><nav aria-label="页面导航"><a className="ws-nav-active" href="#navigation"><LayoutGrid size={14} /> 导航</a><a href="#discover">发现</a><a href="#notes">随手记</a></nav><div className="ws-top-actions"><span className="ws-top-status"><span className="ws-dot" /> MY PERSONAL SPACE</span><button className="ws-icon-button" aria-label={dark ? '切换浅色外观' : '切换深色外观'} onClick={() => { setDark(!dark); try { localStorage.setItem('king:appearance', dark ? 'light' : 'dark'); } catch {} }}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button><span className="ws-avatar">K</span></div></header>
    <main className="ws-main" id="top"><section className="ws-welcome"><div><div className="ws-eyebrow">A LITTLE SPACE. A LOT OF POSSIBILITIES.</div><h1>在这里，开启你的每一天<span>。</span></h1><p>常用网站、值得关注的新鲜事，还有下一刻的灵感。</p></div><a href="#notes" className="ws-welcome-link"><Plus size={15} /> 记录一个想法</a></section>
      <div className="ws-overview"><DateCard /><WeatherCard defaultCity={defaultCity} /><MarketsCard initialStocks={initialStocks} /></div>
      <TasksCard refresh={refresh} />
      <section className={`ws-card ws-navigation ${expanded ? 'is-expanded' : ''}`} id="navigation"><div className="ws-section-heading"><div><span className="ws-eyebrow">YOUR DAILY SHORTCUTS</span><h2><Compass size={21} /> 网站导航 <span className="ws-count">{links.length}</span></h2></div><div className="ws-search"><Search size={17} /><input ref={searchRef} value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索网站、工具或灵感…" aria-label="搜索网站" />{query ? <button aria-label="清空搜索" onClick={() => setQuery('')}><X size={14} /></button> : <kbd><Command size={11} /> K</kbd>}</div></div>
        <div className="ws-bookmarks-body"><aside className="ws-categories" aria-label="网站分类">{['全部', ...categoryNames].map((name, index) => <button key={name} aria-pressed={category === name} className={category === name ? 'active' : ''} onClick={() => { setCategory(name); setSub('全部'); }}><span>{index === 0 ? <LayoutGrid size={16} /> : index % 3 === 0 ? <Globe2 size={16} /> : index % 3 === 1 ? <Sparkles size={16} /> : <Bookmark size={16} />}{name === '全部' ? '全部网站' : name}</span><small>{name === '全部' ? links.length : links.filter(link => link.category1 === name).length}</small></button>)}<div className="ws-sidebar-note"><span className="ws-dot" /> 来自你的 Notion 收藏</div></aside>
          <div className="ws-links-area"><div className="ws-subcategories"><button className={sub === '全部' ? 'active' : ''} onClick={() => setSub('全部')} aria-pressed={sub === '全部'}>全部</button>{subs.map(name => <button key={name} className={sub === name ? 'active' : ''} aria-pressed={sub === name} onClick={() => setSub(name)}>{name}</button>)}</div><div className="ws-result-label"><span>{query ? `“${query}” 的搜索结果` : category === '全部' ? '好工具，让工作轻一点。' : category}</span><span>{filtered.length} 个网站</span></div><div className="ws-link-grid">{filtered.map(link => { const url = safeUrl(link.url); if (!url) return null; return <a className="ws-link" href={url} target="_blank" rel="noreferrer" key={link.id}><LinkIcon link={link} /><div><h3>{link.name}</h3><p>{link.desc || link.category2 || new URL(url).hostname}</p><span>{link.category2 || link.category1}</span></div><ArrowUpRight className="ws-link-arrow" size={14} /></a>; })}</div>{!filtered.length && <div className="ws-empty"><Search size={28} /><h3>{links.length ? '没有找到匹配的网站' : '这里等待你的第一份收藏'}</h3><p>{links.length ? '试试其他关键词，或换个分类看看。' : '在 Notion 导航链接中添加网站后，它们会出现在这里。'}</p><button className="ws-text-button" onClick={() => { setQuery(''); setCategory('全部'); setSub('全部'); }}>查看全部</button></div>}</div></div>
