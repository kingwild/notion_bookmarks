...
function NewsCard({ source, refresh }: { source: typeof NEWS_SOURCES[number]; refresh: number }) {
  ...
  return <article className="ws-card ws-news-card"><header><span className="ws-source-mark" style={{ background: source.color }}>{source.mark}</span><div><h3>{source.name}</h3><p>{source.label}</p></div><a href={source.url} target="_blank" rel="noreferrer" aria-label={`打开${source.name}`}><ArrowUpRight size={17} /></a></header>
    {loading && !feed ? <div className="ws-news-loading" aria-label="加载榜单中">{[1,2,3,4,5].map(n => <div key={n} />)}</div> : feed?.items.length ? <ol tabIndex={0} aria-label={`${source.name}榜单，可上下滚动`}>{feed.items.slice(0, 50).map((item, index) => <li key={item.url}><span className={index < 3 ? 'ws-rank-top' : ''}>{String(index + 1).padStart(2, '0')}</span><button className="ws-news-title" onClick={() => setSelected(item)} title={item.detail || item.title}>{item.title}</button></li>)}</ol> : <div className="ws-feed-empty"><Globe2 size={24} /><p>{feed?.message || '暂时无法读取榜单'}</p><span>可以前往原站查看最新内容</span><div><button onClick={() => setRetry(v => v + 1)}>重试</button><a href={source.url} target="_blank" rel="noreferrer">打开原站 <ArrowUpRight size={12} /></a></div></div>}
    <footer><span>{feed?.status === 'ok' ? `共 ${feed.items.length} 条 · ${new Date(feed.fetchedAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}` : '独立数据源'}</span><a href={source.url} target="_blank" rel="noreferrer">查看完整内容 <ArrowUpRight size={11} /></a></footer>{selected && <NewsOverview item={selected} source={source.id} onClose={() => setSelected(null)} />}</article>;
}
...
export default function Workspace({ links, categories, title, defaultCity, initialStocks, notesEnabled }: Props) {
  ...
  return <div className={`king-workspace ${dark ? 'ws-dark' : ''}`}><div className="ws-ambient" aria-hidden="true" /><header className="ws-topbar"><a className="ws-brand" href="#top"><span className="ws-brand-symbol">K<span>·</span></span><span>KING<span className="ws-brand-light"> / 工作基地</span></span></a><nav aria-label="页面导航"><a className="ws-nav-active" href="#navigation"><LayoutGrid size={14} /> 导航</a><a href="#discover">发现</a><a href="#notes">随手记</a></nav><div className="ws-top-actions"><span className="ws-top-status"><span className="ws-dot" /> MY PERSONAL SPACE</span><button className="ws-icon-button" aria-label={dark ? '切换浅色外观' : '切换深色外观'} onClick={() => { setDark(!dark); try { localStorage.setItem('king:appearance', dark ? 'light' : 'dark'); } catch {} }}>{dark ? <Sun size={18} /> : <Moon size={18} />}</button><OwnerLogin owner={owner} onChange={() => setOwnerRefresh(v => v + 1)} /></div></header>
    <main className="ws-main" id="top"><section className="ws-welcome"><div><div className="ws-eyebrow">A LITTLE SPACE. A LOT OF POSSIBILITIES.</div><h1>在这里，开启你的每一天<span>。</span></h1><p>常用网站、值得关注的新鲜事，还有下一刻的灵感。</p></div><a href="#notes" className="ws-welcome-link"><Plus size={15} /> 记录一个想法</a></section>
      <div className="ws-overview"><DateCard /><WeatherCard defaultCity={defaultCity} /><MarketsCard initialStocks={initialStocks} owner={owner} syncRefresh={refresh} /></div>
